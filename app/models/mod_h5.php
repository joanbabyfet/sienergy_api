...
class mod_h5 extends mod_model
{
    //主键
    ...
    protected function list_data(array $conds)
    {
        ...
        $rows = self::get_list([
            'fields'    => ['id', 'name', 'content', 'status', 'create_user', 'create_time'
            ],
            'where'     => $where,
            'page'      => $page,
            'page_size' => $page_size,
    ...
    //获取绝对h5 url
    protected function h5_url($id)
    {
        $h5_url = config('global.h5_url');
        return sprintf($h5_url, $id);
    }
}
